...
//!
//! The bounded download/cache engine itself lives in the child modules:
//!
//! - [`engine`] — the job orchestrator: admission, resumable receive,
//!   cancellation, source supersession, licence reconciliation, and
//!   restart recovery from the durable journal.
//! - [`storage`] — the only filesystem boundary: derived cache keys,
//!   same-directory temp reservation, the `fsync`'d per-job journal,
//!   verify-then-publish atomic rename, and content-aware unlink.
//! - [`quota`] — the pure eviction/quota policy: global bound, admission
//!   gates, and the newest-first-within-source / oldest-first-across-
//!   sources eviction order.
//! - [`catalog`] — the read-only committed-row index that resolves a
//!   `MediaKey` to [`OfflineCatalogueEntry::LiveOnly`], [`OfflineCatalogueEntry::Cached`],
//!   or [`OfflineCatalogueEntry::Revoked`].
