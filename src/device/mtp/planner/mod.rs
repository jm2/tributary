...
/// What the browse walk admitted, in stable browse order.
struct AdmittedStages {
    mtp_stages: Vec<MtpTransferStage>,
    staging_writes: Vec<MtpStagingWrite>,
...
impl MtpTransferPlanner {
    ...
    pub fn plan(&self, request: &MtpTransferRequest) -> Result<MtpTransferPlan, MtpPlanError> {
        ...
        let admitted = Self::stage_admitted_files(request)?;
        if admitted.transfer_items.is_empty() {
            return Err(MtpPlanError::EmptyBrowse);
        }

        Ok(MtpTransferPlan {
            mtp_stages: admitted.mtp_stages,
            staging_writes: admitted.staging_writes,
            transfer_items: admitted.transfer_items,
            expected_total_bytes: admitted.total_bytes,
            device_id: request.session.device_id().clone(),
            storage_id: request.storage.storage_id,
        })
    }

    /// Reject a request before any staging work: session liveness, a
    /// budget that admits work, a real staging root, a relative
    /// destination root, and a non-empty browse.
    fn validate_plan_request(request: &MtpTransferRequest) -> Result<(), MtpPlanError> {
    ...
    /// Walk the objects in browse order (stable depth-first), admitting
    /// every regular file that fits the budget.
    ///
    /// MTP handles are opaque and not guaranteed sequential, so sorting
    /// by raw handle would break the browser's directory-local
    /// traversal order instead of restoring it.
    fn stage_admitted_files(request: &MtpTransferRequest) -> Result<AdmittedStages, MtpPlanError> {
        let mut mtp_stages: Vec<MtpTransferStage> = vec![
            MtpTransferStage::OpenSession,
            MtpTransferStage::BrowseStorage,
        ];
...
fn staging_path_for(
    ...
    // The storage id is part of the staging identity: two storages on
    // one device can expose identical parent chains and object names,
    // and without this component they would stage to the same path and
    // silently overwrite each other.
    path.push(format!("storage-{}", request.storage.storage_id));
...
fn destination_path_for(
    ...
    // builder added. The `storage-<id>` component that follows it is
    // intentionally kept: a transfer rooted at one destination that
    // pulls from two storages of the same device must not let the
    // second storage silently overwrite the first. The destination is
    // rooted beneath `request.destination_relative_root`, joined
    // component by component with `PathBuf::push` so non-UTF-8 device
    // names survive intact and the platform separator is used
    // throughout instead of a hardcoded `/`.
