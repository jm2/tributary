...
impl MtpTransferPlanner {
    ...
    pub fn plan(&self, request: &MtpTransferRequest) -> Result<MtpTransferPlan, MtpPlanError> {
        ...
        let (mtp_stages, staging_writes, transfer_items, total_bytes) =
            Self::stage_admitted_files(request)?;
    ...
    fn stage_admitted_files(
        request: &MtpTransferRequest,
    ) -> Result<
        (
            Vec<MtpTransferStage>,
            Vec<MtpStagingWrite>,
            Vec<TransferItem>,
            u64,
        ),
        MtpPlanError,
    > {
        let mut mtp_stages: Vec<MtpTransferStage> =
            vec![MtpTransferStage::OpenSession, MtpTransferStage::BrowseStorage];
        ...
        Ok((mtp_stages, staging_writes, transfer_items, total_bytes))
    }
