...
pub struct MtpSession {
    ...
    /// Transport-installed liveness probe. [`MtpSession::verify`] calls
    /// it so disconnect, replacement, and transport teardown surface as
    /// [`MtpTransportError::SessionLost`] instead of a stale-Ok check.
    liveness: Arc<dyn Fn() -> Result<(), MtpTransportError> + Send + Sync>,
}

impl MtpSession {
    /// Construct a session from the transport's resolved identity, a
    /// backend cookie, and a liveness probe that answers "is this
    /// session still the live binding to its device?". The constructor
    /// is `pub(crate)` so only the transport can mint a session.
    pub(crate) fn new(
        device_id: MtpDeviceId,
        descriptor: MtpUsbDescriptor,
        backend_token: Box<dyn std::any::Any + Send + Sync>,
    ...
    /// Re-verify the session is still bound to a live device. The check
    /// is transport-backed: the transport installed a liveness probe
    /// when it minted the session, so a device that detached, a device
    /// that was replaced (any newer transport binding supersedes this
    /// session), or a transport that was dropped all surface as
    /// [`MtpTransportError::SessionLost`]. The planner will surface
    /// that error rather than commit partial work.
...
pub mod test_transport {
    ...
    ///
    /// Session liveness is transport-backed: the transport counts
    /// binding generations in `session_epoch` and bumps it on every
    /// mutation (`open_session`, object/detach helpers). A session
    /// minted under an older generation fails verification, so a
    /// detached device, a replaced device, and a superseding session
    /// all produce [`MtpTransportError::SessionLost`].
    ...
    struct InMemoryShared {
        ...
        /// Binding generation. Sessions capture the value current at
        /// mint time; any transport mutation invalidates them.
    ...
    impl InMemoryMtpTransport {
        ...
        /// composes naturally. Mutating the transport bumps the binding
        /// generation, invalidating any outstanding sessions.
        pub fn add_object(
            &self,
            device_index: usize,
            storage_index: usize,
            ...
            self.shared.session_epoch.fetch_add(1, Ordering::SeqCst);
        }

        /// Detach every device, as if the user unplugged them. Any
        /// outstanding session fails verification afterwards.
        pub fn detach_devices(&self) {
            let mut state = self.shared.state.lock().expect("transport mutex");
            state.devices.clear();
            self.shared.session_epoch.fetch_add(1, Ordering::SeqCst);
        }

        /// Attach a device, as if the user (re)plugged it. Re-attaching
        /// a device with the same descriptor is a *replacement*, not a
        /// resurrection: sessions minted before the detach stay lost
        /// because the binding generation advanced.
        pub fn attach_device(&self, device: InMemoryDevice) {
            let mut state = self.shared.state.lock().expect("transport mutex");
    ...
    impl MtpTransport for InMemoryMtpTransport {
        ...
        fn open_session(
            ...
            // Opening a session supersedes every outstanding binding:
            // capture the generation AFTER the bump so this session's
...
mod tests {
    ...
    #[test]
    fn session_verify_fails_after_device_detaches() {
        let transport = InMemoryMtpTransport::single_device(descriptor("ABC123"));
        let session = transport
            .open_session(&descriptor("ABC123"))
            .expect("session");
        ...
        assert!(matches!(
            session.verify(),
            Err(MtpTransportError::SessionLost(_))
        ));
    }

    #[test]
    fn session_verify_fails_when_device_is_replaced() {
        let transport = InMemoryMtpTransport::single_device(descriptor("ABC123"));
        let old_session = transport
            .open_session(&descriptor("ABC123"))
            .expect("session");
        // Unplug, then re-plug the same physical descriptor. The old
        // session must stay lost even though a device with the same
        // identity is present again: the transport's newer binding
        // supersedes it.
        transport.detach_devices();
        transport.attach_device(test_transport::InMemoryDevice {
        ...
        // A session opened against the re-attached device is live.
        let new_session = transport
            .open_session(&descriptor("ABC123"))
            .expect("session");
        assert!(new_session.verify().is_ok());
    }

    #[test]
    fn session_verify_fails_after_transport_drop() {
        let transport = Arc::new(InMemoryMtpTransport::single_device(descriptor("ABC123")));
        let session = transport
            .open_session(&descriptor("ABC123"))
            .expect("session");
        assert!(session.verify().is_ok());
