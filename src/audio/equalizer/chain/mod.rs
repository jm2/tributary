...
impl EqChain {
    ...
    pub(crate) fn complete_pending_limiter_edit(&mut self, installed: bool) -> bool {
        ...
        if installed {
            ...
            //
            // The callback's removal is also *physical*: it unlinks the
            // limiter from both sides, restores the direct
            // eq → post-convert route, and drops the element from the
            // bin. The fixture must mirror that, or the bin keeps an
            // orphaned, still-linked `clipper` whose pads block every
            // later real edit — the stale-completion re-issue
            // (refinery round 4, PR 220 A1) exercises exactly such a
            // follow-up insert.
            if let Some(clipper) = self.bin.by_name("clipper") {
