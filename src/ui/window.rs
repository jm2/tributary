...
// ── Audio output persistence (outputs.json) ─────────────────────────
...
fn load_saved_outputs() -> Vec<SavedOutput> {
    outputs_json_path()
        .and_then(|p| std::fs::read_to_string(p).ok())
...
fn add_saved_output(output_type: &str, name: &str, host: &str, port: u16) {
...
#[allow(dead_code)]
fn remove_saved_output(host: &str, port: u16) {
    let mut outputs = load_saved_outputs();
    let key = format!("{host}:{port}");
...
fn show_add_output_dialog(window: &adw::ApplicationWindow, output_list: &gtk::ListBox) {
    let dialog = adw::AlertDialog::builder()
