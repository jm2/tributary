...
[33781875201](https://github.com/jm2/tributary/actions/runs/33781875201), test workflow in
PR #224), so those two boxes are complete and counted again. The withdrawn P2.6 false finding,
section-summary, and global-validation gate boxes are still not task progress:
**222/225 (98.7%)** in-scope checklist items complete: **52/52 P0**, **64/64 P1**, **76/79 P2**,
and **30/30 P3**. This incorporates the four P2.9 boxes closed by PR #99
...
PR #124 named every PE-inspector argument and passed its native x86_64 and ARM64 package/probe jobs,
but the 2026-07-18 affected-host rerun still rejected the singleton Soup target after dependency
discovery, release compilation, plugin synchronization, scanner replacement, and distribution-path
resolution. That exact result disproves the positional array-binding hypothesis; explicit names
were not a complete repair. Merged PR #127 removes the non-terminal `[string[]]` function boundary
entirely. Callers now construct explicit `List[string]` instances for the Soup singleton,
each closure round, and every bounded process batch. Before `llvm-readobj` receives a target, the
validator distinguishes null/empty values, quotes or control characters, non-rooted paths,
`GetFullPath` failures, non-DLL/EXE extensions, and missing files. Its bounded single-line
diagnostic retains at most 192 sanitized target characters plus a fixed truncation marker. The
nonexecuting inspector, exact architecture, batch/argument/output/time/process-tree limits, and
fail-closed closure policy are unchanged.
PowerShell 7 exercises the behavior regression away from Windows, while native run `29648906031`
passed the x86_64 and ARM64 package/probe jobs and the x86_64 job's required Desktop PowerShell 5.1
behavior regression. PowerShell's AST parser reports zero errors; formatting, diff, locked check,
and strict debug/release Clippy pass; all 14 `windows_*` and three `powershell_*` tests pass; and
...
source loss, while a stale/superseded dialog cannot suppress ordinary fallback. Automatic
construction failure or later source loss returns a selected radio lane to Local and restores the
...
source session epoch. A native non-UTF-8 leaf name becomes bounded lossy Unicode only as a
parser/presentation hint; the already-open handle remains the sole authority. Playback receives an
inseparable lease-bearing file capability from `SourceRegistry`, rechecks the lease before and
after every handle clone, and starts embedded-art parsing only after the selected output accepts
that retained file. Cursor-based tag and artwork consumers serialize per capability while every
output retains position-independent proxy reads. A pre-source admission generation rejects stale
work after a newer OS delivery, any explicit playback intent (including scrubber seeks), Stop, a
real output change, or shutdown. Queue
replacement, unrepeated EOS, playback/load error, output transfer, and shutdown explicitly retire
the ephemeral source; exact output reselection remains inert. Independent review tightened the
admission gate predicate, made playback intent and scrubber invalidation explicit, limited baseline
clears to hidden sources with an actual UI projection, serialized external publication against
shutdown, and kept raw adapter results inseparable from their lifecycle lease. Automated review
then made the parser hint tolerant of native non-UTF-8 leaf names and serialized cursor-based
...
debug suite passes 20 library, 926 application, and 10 repository-metadata tests (**956 total**),
...
- [x] Record implementation: workflow contract implemented in PR #68; the
  ...
  verification" both passed: `build_ref` resolved to `74b4279`, the immutable commit of
...
- [ ] Record implementation and manual validation: symlink containment landed in commit `1886847`;
  ...
  lifecycle; and PR #129 supplies registry-owned scanning, pathless epoch-bound
  catalogue/queue identity, retained mounted-root/file authority, exact membership-gated at-use
  playback and artwork, lifecycle-ordered disconnect/reconnect/removal, and retained background
  scan-failure replay when an inactive device is later selected. The current branch
  passes locked all-target/all-feature check, strict debug Clippy, formatting, diff checks, focused
  removable identity/authority/adapter/registry/UI coverage, and the complete 956-test serial debug
...
The fallback was architecturally backwards, and it was the *default* path on a typical Linux box:
the code expected a `raopsink` element that current supported packages do not ship, so
`airplay_output.rs:172-175` routed to `build_shairport_pipeline` in the common case. That
function opened by discarding the receiver the user selected —
`airplay_output.rs:289-295`: `let _ = (host, port); // shairport-sync uses its own discovery.` —
and then piped PCM into `shairport-sync`, which is an AirPlay **receiver**, not a sender. It
could not transmit to the device that was clicked.
...
three AirPlay sender design, implementation, and validation entries under `P2.4 — Audio processing
and output protocols` in `docs/task.md`. Those entries first resolve the non-shipped `raopsink`
seam, then select and implement a maintained sender path. Until that lands, the localized error is
the truthful behavior, not actionable install guidance.
...
- [x] Prove the packaged Windows artifact carries and selects the required GStreamer runtime and
  ...
  inspection with the same fixed PE-target diagnostic. PR #124 named all seven arguments to the
  non-terminal `[string[]] $Paths` boundary and passed both native CI package/probe jobs, but the
  subsequent exact Windows PowerShell/MSYS2 rerun failed at the same singleton validation. That
  result disproves positional array binding as the cause. The current candidate replaces the
  function-boundary array with an explicit `List[string]` assembled by the caller for the Soup
  singleton, each closure round, and every bounded inspector batch. Per-target validation now
  identifies null/empty input, quotes or control characters, a non-rooted value, a
  `GetFullPath` failure, a non-DLL/EXE extension, or a missing file before process launch. Its bounded
  single-line diagnostic retains at most 192 sanitized target characters plus a fixed truncation
  marker. The nonexecuting architecture-local `llvm-readobj` path and all existing command-line,
  process-tree, batch, and whole-closure limits remain unchanged. The current focused filters pass
  14 `windows_*` and three `powershell_*` tests; the target-batch behavior regression uses `pwsh`
  away from Windows and is designed to require Desktop PowerShell 5.1 on native Windows. AST,
  formatting/diff, locked check, strict debug/release Clippy, and both complete 926-test profiles
  pass. Native package CI/Desktop PowerShell 5.1 execution and the exact affected-host rerun remain
  required before calling this fixed. This changes no import target, runtime-copy policy,
  dependency, lockfile, or checklist arithmetic.
...
- [x] Centralize source refresh, cancellation, disconnect, and failure state. The production
  ...
  sanitized failure, immutable snapshot, keyed
  ...
  Each accepted OS-open file is a hidden, ephemeral one-track source with a registry-owned
  provenance claim, exact session epoch, random source/track identity, and retained open file.
  Exact-handle tag parsing and bounded metadata validation happen before identity is minted; failed
  candidates publish no source. The registry returns only a typed lease-bearing file stream and
  rechecks the exact media lease around handle cloning, so callers never receive separable raw
  adapter data and lease parts. External publication and shutdown share one admission gate: shutdown
  closes the gate before draining claims, while a candidate that loses the gate rolls back instead
  ...
  suite passes 20 library, 926 application, and 10 repository-metadata tests (**956 total**), with
...
- [x] Record implementation: P1.6 completed the remote resolver/session ownership subset in PR
  ...
  intent/terminal/output/shutdown retirement. PR #129 completes the record with a
...
locked debug suite passes 20 library, 926 application, and 10 repository-metadata tests
(**956 total**) with zero failures or ignored tests. Focused coverage includes 21 mounted-root
authority tests, 22 removable-filtered identity/UI/registry tests, five adapter tests, and four
...
910 application, and 10 repository-metadata tests (**940 total**) with zero failures or ignored
...
and simplified the bounded optional-extension parser. The same review's missing-tag concern is
already covered by the shared parser's `Unknown Artist`/`Unknown Album` defaults and the untagged WAV
regression. Its shared-cursor concern does not reach playback: every Local, AirPlay, Chromecast, and
MPD retained-file load uses the app-owned HTTP proxy's position-independent `read_at`/`seek_read`
producer. It did reveal a narrower overlap between stale and current artwork jobs, so a
per-capability mutex now covers each complete cursor-based clone-and-parse operation; a deterministic
two-worker regression proves the second consumer cannot enter until the first releases it. The
reported `radio_id` binding remains live in radio baseline and fallback calculations.
...
held, so source identity cannot be minted for a delivery superseded during parsing. All explicit
playback intents—including the progress scrubber—now invalidate an in-flight OS delivery. The
baseline reducer clears hidden lifecycle entries only when an ordinary UI projection was actually
published or is still UI-owned, preventing hidden external adoption from invalidating itself.
...
loss, while stale/superseded requests cannot suppress real fallback. Final GTK review also found
that automatic radio loss restored Local rows without restoring the user's music columns or browser;
the authoritative fallback now restores both presentation preferences before rendering Local. This
Automated PR review also removed repeated Haversine/centroid work from sort comparisons by caching
one distance per accepted station. This slice closes another part of P3.1's compound final record
without changing literal arithmetic:
**219/223 (98.2%)** overall,
...
Accepted validation (2026-07-18, PR #127 P2.11 Windows typed PE-target repair):
PowerShell 7's AST parser reports `parser_errors=0`; `cargo fmt --all -- --check`,
`git diff --check`, locked all-target/all-feature check, and strict debug/release Clippy pass. The
focused filters pass 14 `windows_*` and three `powershell_*` platform-runtime tests. The
new target-batch behavior regression executes through `pwsh` on non-Windows development hosts and
requires Desktop PowerShell 5.1 on native Windows. Complete locked debug and release suites each
pass 20 library, 896 application, and 10 repository-metadata tests (**926 total**) with zero
failures or ignored tests. CI run `29648906031` passed both native x86_64 and ARM64 Windows package
and probe jobs; the x86_64 job also executed the behavior regression under Desktop PowerShell 5.1.
That cross-host evidence does not substitute for the exact affected Windows PowerShell/MSYS2 rerun,
which remains pending. Consequently the tracker remains **219/223 (98.2%)** overall, **76/79 P2**,
and **29/30 P3**.
...
- 2026-07-18 — P3.1 treats each OS `open` delivery as one ordered playback intent, not as a set of
  ...
  catalogue projection; reducer baselines therefore clear a hidden source only when the UI actually
  published or still owns a projection, and playback—not visibility observation—owns external
  retirement.

  The pathless queue binds the exact random source ID, track ID, and session epoch. Lifecycle
  resolution attaches `MediaLease` inside the adapter boundary and returns one inseparable retained
  file capability; no registry caller can hold raw adapter output separately from its lease. Each
  handle clone rechecks revocation before and after cloning, and embedded art starts only after the
  output accepts the same retained capability. A newer OS delivery, visible track selection, Play,
  Pause/toggle, Next, Previous, explicit retry, progress-scrubber seek, Stop, real output transfer,
  or shutdown invalidates in-flight admission. Automatic EOS replay/advance deliberately does not.
  Replacing an external queue, unrepeated EOS, output/load error, real output change, and shutdown
  retire the exact hidden source idempotently; selecting the already-active output remains a
  complete no-op. Independent review added the exact gate predicate,
  explicit playback-intent/scrubber coverage, the hidden UI-owner filter, shutdown/publication race
  serialization, and the nonseparable lease-bearing resolver boundary before this slice was
  accepted for PR #128.
...
- 2026-07-14 — P1.6's receiver-ticket slice classifies every legacy/direct media URI before it can
  ...
  removes the final local/playlist path-based consumer. Radio-Browser and external-file at-use
  adapters subsequently landed; P3.1 now needs only the removable-file locator adapter.
...
- 2026-07-17 — P2.11 resolves the completed Windows distribution through PowerShell's filesystem
  ...
  evidence.
- 2026-07-18 — P2.11 treats PR #124's exact-host reproduction as evidence against its positional
  array-binding hypothesis, not as a reason to weaken PE-target validation. Merged PR #127
  removes `[string[]]` from the function boundary and has each caller construct an explicit
  `List[string]` for the singleton Soup check, a closure round, and every bounded process batch.
  Each target is rejected before process launch if it is null/empty, contains a quote or control
  character, is not rooted, cannot survive `GetFullPath`, is not a DLL/EXE, or is not an
  existing file. The bounded single-line failure report preserves at most 192 sanitized target
  characters plus a fixed truncation marker, so the affected host can identify the exact
  predicate. The selected architecture's nonexecuting `llvm-readobj`, closure inputs, and all
  established batching, argument, output, deadline, and process-tree bounds remain unchanged. A
  live behavior regression uses `pwsh` on non-Windows hosts and Desktop PowerShell 5.1 on Windows.
  CI run `29648906031` passed both native Windows package/probe jobs and exercised that regression
  under Desktop PowerShell 5.1 on x86_64; neither that cross-host coverage nor native package CI
  substitutes for rerunning the exact affected PowerShell/MSYS2 command. Checklist arithmetic
  therefore remains unchanged until the separate live-playback task is actually satisfied.
...
- 2026-09-03 — P0.7's live manual-dispatch acceptance ran after all. The release dry-run
  ...
  `build_ref` to `74b4279` — the immutable commit of `refs/tags/v0.6.2` — derived the
  matching version, and skipped every build/publish job. The two P0.7 verification boxes
  are checked and counted, completing P0 at 52/52 and moving the tracker to 222/225;
...
| 2026-07-16 | P2.9 | PR #99 | Removed the inoperative `shairport-sync` receiver fallback, moved the remaining capability refusal ahead of media preparation, localized a missing-`raopsink` error that then recommended installing `gst-plugins-bad`, surfaced safe player errors in-app, and added focused missing-element/load-path regressions. PR #152's later provenance review established that current supported packages do not ship `raopsink`, removed that recommendation, and changed the current behavior to an honest unavailable-sender refusal. A maintained packaged sender remains open under P2.4. |
...
| 2026-07-18 | P2.11 Windows PE-import argument-binding repair (follow-up) | PR #124 | Replaces positional calls to the non-terminal `[string[]] Paths` parameter with explicit names for every singleton and batch inspector argument. The regression checks the declaration and each of the exactly two production calls independently. PowerShell 7 parses the complete script, all 11 focused Windows contracts pass, and locked debug/release suites each pass 20 library, 866 application, and 10 repository-metadata tests (896 total) with locked check and strict Clippy green. CI run `29633729566` passed every job, including production bundle/probe execution on native x86_64 and ARM64 Windows runners. The later exact affected-host rerun nevertheless failed at the same singleton Soup validation, disproving the positional array-binding hypothesis and showing that named arguments were not a complete repair. The separate live DAAP/Subsonic proof remains pending, so checklist arithmetic stays at 219/223 overall and 76/79 P2. |
