...
coordinator API. Retirement first closes operation admission, drains admitted work, retires the
owner, and publishes one immutable result shared by close, rebind, and shutdown waiters before any
successor becomes Active. Poison, operation-capacity failure, and a closed runtime fail the bridge
terminally rather than admitting a successor. A predecessor Clear remains owed when a successor
NowPlaying is rejected by source policy or a busy runtime and is cancelled only after the
successor crosses runtime admission successfully; qualified Enqueue remains durable.
...
- [ ] Implement Last.fm authorization and protected secret storage, now-playing/scrobble thresholds,
  ...
  - The Active bridge preserves owner-before-runtime ordering and holds each admitted operation
    through exact source-policy revalidation and runtime handoff. Retirement closes new operation
    admission, drains accepted work, retires the owner and submits any required Clear, then publishes
    one immutable result shared by all concurrent close, rebind, and shutdown callers. A successor
    window or activation cannot become live before that result. Owner, operation-gate, retirement,
    and coordinator poison paths fail closed; a closed runtime is terminal rather than allowing a
    successor activation after an unproven Clear. Focused race tests cover lazy-build retirement,
    in-flight dispatch, close/rebind/shutdown joining, shared success and failure, terminal poison,
    and Clear-before-successor ordering.
  ...
  suites of 20 library, 1,672 application, and 14 repository-metadata tests (1,706 total), including
  focused ingress-capability, activation-epoch, lazy-builder, real-runtime durable-enqueue,
  exact-managed-source revocation, dispatch-order, source/runtime-rejection, retirement-drain,
  shared-result, rebind/shutdown, poison, and predecessor-Clear race regressions. Strict Clippy is
...
| Date | Task | PR | Result |
|---|---|---|---|
| 2026-07-22 | P2.1 Last.fm sealed headless runtime bridge | Current branch | Added an exact-window/activation-epoch Active lease around the existing process coordinator, with a coordinator-private mint for its sole playback owner and a one-shot non-`Clone` runtime capability exposing only bounded NowPlaying, Enqueue, and Clear admission. Accepted-load construction is lazy, operation-drained, rechecked before owner mutation, and explicitly bounded/non-reentrant with lifecycle drains. Exact managed-source policy is revalidated before owner-to-runtime dispatch. Retirement revokes admission, drains in-flight work, sends any required Clear, and publishes one immutable result shared by close, rebind, and shutdown before a successor can activate; poison and a closed runtime fail terminally. Ephemeral ordering preserves a predecessor Clear across source-rejected or runtime-Busy successor NowPlaying and cancels it only after successful successor admission. A real actor/claimed-ingress regression proves coordinator-generated Enqueue reaches durable SQLite, while a live registry-bound managed source proves authority loss emits one Clear and rejects its stale reference. Locked debug/release suites each pass 1,706 tests, with strict Clippy in both profiles, fuzz Clippy, Rust 1.92 all-target, formatting, diff, and dependency-audit gates green apart from the two documented allowed unmaintained warnings. Production startup still leaves the coordinator Dormant and starts no Last.fm runtime, claims no playback ingress, issues no activation, and wires no UI/auth/live policy. P2.1 stays open at 14/38 (36.8%). |
