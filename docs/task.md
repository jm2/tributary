...
action's bot allowlist was empty. The workflow now explicitly admits every GitHub Bot/App actor,
covering dependency automation and GasTown when GitHub classifies it as non-User, while leaving
ordinary non-write `User` actors behind the repository-permission gate. A repository metadata
contract prevents either the wildcard from disappearing or the broader non-write-user bypass from
being enabled accidentally. P2.1 remains the feature focus at **14/38 (36.8%)**.
...
| Date | Task | PR | Result |
|---|---|---|---|
| 2026-07-28 | Claude PR review bot admission | [#186](https://github.com/jm2/tributary/pull/186) | Allowed every GitHub Bot/App actor to enter the pinned automatic Claude review, fixing Dependabot's pre-review CI failure and covering GasTown automation when classified as non-User. Ordinary non-write `User` actors remain permission-gated, and a repository metadata regression pins both sides of that boundary. This CI reliability fix does not advance the 14/38 feature numerator. |
