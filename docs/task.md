...
- [ ] Make Recently Played and Top 25 reflect the new history contract deterministically, including
  ...
  engine atomically updates only that stable `TrackId`, repairs incompatible null or negative
  counts to the first legitimate play, saturates `play_count` at `i32::MAX`, keeps the greatest
...
| 2026-07-18 | P1.3 authoritative playback-history persistence | [#135](https://github.com/jm2/tributary/pull/135) | Bound one progress latch to each exact local queue occurrence independently of output generations; rejected/stale/retry events cannot double count, paused polls stay inert, Repeat One rolls back only its tentative occurrence before generation handoff, and other discontinuities re-anchor. Added a shared shutdown admission gate plus FIFO drain, atomic stable-ID count/timestamp persistence with defensive null/negative repair, post-commit Plays refresh and playlist-projection invalidation, plus generation-scoped AirPlay position evidence. Deterministic Recently Played and Top 25 behavior remains the final P1.3 record. |
