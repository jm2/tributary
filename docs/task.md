...
stale. The sole advertised Claude built-in is also bare-denied, so the model has no usable tools;
the job supplies no OIDC authority or broader App token and accepts only one bounded
schema-validated string. A fixed trusted step revalidates the revision, indents the result into an
inert text block so links, mentions, HTML, and bot commands cannot activate, and then publishes it
with the short-lived job token. Ordinary non-write `User` actors remain behind the
repository-permission gate. A semantic repository metadata contract pins the actor-consistent
direct/trusted split, exact-revision input, environment-safe cap, trusted-default checkout,
wildcard, tool and output containment, stale-result checks, and absence of the broader
non-write-user bypass.
...
| Date | Task | PR | Result |
|---|---|---|---|
| 2026-07-28 | Claude PR review bot admission | [#186](https://github.com/jm2/tributary/pull/186) | Split automatic review by the triggering actor into a direct `User` path and a trusted bot/App `workflow_run` follow-up, fixing both Dependabot's empty allowlist rejection and the unavailable Actions Claude credential that would have failed next. The bot path starts when CI is requested, admits every Bot/App including GasTown automation classified as non-User, checks out only the immutable trusted default-branch SHA, and fetches an exact base/head comparison capped below the runner's environment-string limit. It supplies no usable tools, OIDC authority, or broader App token; accepts only bounded schema output; revalidates the revision before publishing; discards stale work; and renders model output as inert text through a fixed trusted command. Ordinary non-write `User` actors remain permission-gated. A semantic metadata regression pins all of those boundaries and the absent non-write-user bypass. This CI reliability fix does not advance the 14/38 feature numerator. |
