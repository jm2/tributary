...
and reads the diff only through GitHub. Ordinary non-write `User` actors remain behind the
repository-permission gate. A semantic repository metadata contract pins the direct/trusted split,
base-only checkout, wildcard, and absence of the broader non-write-user bypass. P2.1 remains the
feature focus at **14/38 (36.8%)**.
...
| Date | Task | PR | Result |
|---|---|---|---|
| 2026-07-28 | Claude PR review bot admission | [#186](https://github.com/jm2/tributary/pull/186) | Split automatic review into a direct `User` path and a trusted bot/App `workflow_run` follow-up, fixing both Dependabot's empty allowlist rejection and the unavailable Actions Claude credential that would have failed next. The bot path starts when CI is requested, admits every Bot/App including GasTown automation classified as non-User, pins checkout to the trusted PR base SHA, and treats proposed changes only as GitHub data. Ordinary non-write `User` actors remain permission-gated. A semantic metadata regression pins the routing, checkout, wildcard, and absent non-write-user bypass. This CI reliability fix does not advance the 14/38 feature numerator. |
