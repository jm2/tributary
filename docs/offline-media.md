...
2. **Resumption is exact, validated, and bounded.** The durable journal —
   ...
   the cap is accepted
    ...
    establish the longest complete record prefix, and a torn partial
    record a crash mid-append left behind is resolved by durably
    truncating the sidecar to that prefix (`ftruncate`, then `fsync`). A
    torn tail is not a record: it certifies no byte, and the
...
1. **Quota is global, and the accounting covers every byte the engine
   ...
   segment's offset and length, its SHA-256, and fixed framing — and the
   ...
   **before the step-5 rename**, so a published-but-uncommitted file can
   ...
   - Journal/sidecar metadata is charged and released by the same rules as
     payload, so it is never an uncharged durable write and is never
      ...
      metadata it owns from the journal's complete framed records, charging
      ...
      sidecar's complete record prefix — the first position where the fixed
      framing does not yield a complete record ends the prefix — and resolves
      a torn partial record a mid-append crash left behind before admitting
      ...
      observed — until they become reclaimable. The rule is one rule for
      segment and lifecycle records alike, and it is engine-local: prefix
      validation and truncation need no source authority, discard no complete
      record, and touch no durable owner, so an authority-deferred recovery
   ...
   - **Worked invariant check.** Two unknown-length jobs `A` and `B`
     ...
     **Torn-tail check at exact quota.** Place `A` alone at the exact
     ...
     offset, a torn record tail past the last complete sidecar record,
     and no surviving reservation. Before anything is admitted it
     validates `A`'s sidecar prefix, durably truncates the record tail to
     that prefix — and the temp to the journaled offset, per the
     resumption rule — and re-derives the accounting from complete
     ...
     length equals the charged prefix again. `B` — admitted only after
     ...
     authority, discards no complete record, and leaves no durable owner
     touched, so a deferral never persists with unaccounted bytes, and
     every later pass re-validates the prefix so a fresh tail is
...
| Process crash mid-sidecar-append leaves a torn partial record — segment or lifecycle — past the last complete record | Startup accounting validates the sidecar's complete record prefix before any admission and resolves the torn tail by truncate-or-charge: durably truncated to the valid prefix when possible — a torn tail certifies no byte, so truncation discards nothing certified — otherwise charged at its full physical length to the owning job until a later pass reclaims it. A sidecar that cannot be read, validated, or truncated fails the affected job's admission closed with its bytes held charged at the observed length, or the job's pre-crash upper bound when even the length cannot be observed. The rule is engine-local, so an authority-deferred pass still performs it and never defers with unaccounted bytes on disk. |
