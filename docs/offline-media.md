...
| Capabilities | The remote source owns a default-deny `OfflineSnapshot` capability. Offline eligibility is an independent default-deny decision: it is not coupled to live `ServerPlaylist` read authority (whose `PullSnapshots` opt-in is Subsonic-only) nor to regular-playlist authority; the reviewed opt-ins are Subsonic, Jellyfin, Plex, and DAAP, each via its own authenticated download path. Radio-Browser, removable, external-file, and built-in local sources cannot. | Adapter-specific download strategies beyond HTTP(S) `Range` and Subsonic/Jellyfin/Plex/DAAP download endpoints. |
...
2. Offline eligibility is its own default-deny decision, independent of the
   registry's playlist contracts: live `ServerPlaylist` read authority (whose
   `PullSnapshots` opt-in is currently Subsonic-only per
   [`architecture/source-lifecycle.md`](architecture/source-lifecycle.md)) and
   regular-playlist authority neither confer nor require offline eligibility.
   The reviewed opt-in set is the four credentialed remote adapters —
   Subsonic, Jellyfin, Plex, and DAAP — each justified by the authenticated
   per-track download path documented in the
   [server capability matrix](#server-capability-matrix); any other adapter
   must return `None` until its own ADR adds a reviewed row. Removable,
...
download admission — including the atomic admission that commits a
crash-recovered publication — and reconciliation/retirement. It is never a
playback precondition. A committed snapshot — one whose row was published after
...
| `journal_bytes` | `u64` | Engine-owned journal/sidecar metadata made durable so far: one bounded, fixed-size segment record per committed segment (offset, length, SHA-256, framing) plus the publish-intent, intent-clear, and terminal-verdict lifecycle records. Charged against the same global quota as payload, because it is part of the cache root's engine-owned footprint. The payload-derived segment records are bounded by the segment coalescing rule; the lifecycle records are bounded because a job may begin at most a fixed `publish_attempt_budget` publish attempts ([the publish-intent protocol](#publish-intent-and-restart-recovery)), and they are covered by a single per-job lifecycle reservation sized to that whole bounded worst case, charged once before the job's first byte and held until terminal cleanup, so the mandatory intent, intent-clear, and verdict records — including the final record of an exhausted budget — are always writable at the quota boundary. This field counts only records already durable; the still-unwritten lifecycle obligation is reconstructed as a fresh reservation on restart and is at all times bounded by the same budget, so it can never exceed the reservation already charged. Re-derived from the durable journal, never from a process-local counter, on restart: the re-derivation counts only the sidecar's records whose framing is complete and whose per-record checksum verifies over the record's own body, so neither a torn partial record a crash mid-append left behind nor a full-length frame whose torn body fails that checksum is ever counted as progress, and either is resolved — durably truncated, or charged at its physical length until reclaimable — by the torn-sidecar-record rule before any job resumes or a competitor is admitted. |
...
2. **Resumption is exact, validated, and bounded.** The durable journal —
   ...
   so the durable-before-journal ordering never certifies payload bytes
   beyond the per-track cap or engine-owned bytes beyond the global
   total, and a response whose payload size exactly equals
   the cap is accepted
    ...
    digest, and re-requests the remainder with `Range` **and** `If-Range`
     carrying the captured `resume_validator`. The sidecar is validated at
     the same cut: before the job — or any competing job — is admitted, the
     engine walks the sidecar's fixed record framing from offset zero to
     establish the longest valid record prefix — valid meaning the framing
     yields a complete record **and** the record's per-record checksum
     verifies over the record's own body — and a record a crash mid-append
     left behind is resolved by durably truncating the sidecar to that
     prefix (`ftruncate`, then `fsync`), whether the crash tore a partial
     tail or filled the frame's length and framing bytes while interior
     body sectors never reached disk. A record the checksum fails is not a
     record: it certifies no byte, and the
     durable-before-journal ordering means no journaled progress depends
     on it, so truncation discards nothing certified. It is engine-local —
    it needs no source authority and touches no durable owner, so an
    authority-deferred pass performs it too — and the same rule covers
    segment and lifecycle records alike. When the truncation cannot be
    made durable, every physical tail byte is charged to the owning
    job's footprint at its observed length until a later pass reclaims
    it; when the sidecar cannot be read or its prefix validated, the
    affected job fails admission closed and its last-known upper-bound
    footprint stays charged. The engine never admits a competitor
    against bytes it owns but cannot account, and never releases a
    charge on a guess. A `206` continues the job. A
...
4. **Failure is structured.** `OfflineError` is a typed enum with redacted
   ...
   `QuotaExceeded`, `PublishRetriesExhausted`, `StorageUnavailable`,
   `UnsupportedSource`). `PublishRetriesExhausted` is the defined safe refusal
   when a job has spent its bounded publish-attempt budget on repeated crashes
   in the rename window; it is not a quota failure and never stands for an
   uncharged write. `Denied` is
...
saved source a durable `SourceIncarnationId` and persists it in that
source's saved-source record through the versioned envelope defined
below, exactly as a newly registered source would receive one.

**The saved-source record moves to a version-2 envelope; the new field is
versioned, not smuggled into v1.** `servers.json` is a strict format: the
current `SavedServerEnvelope` accepts only `schema_version = 1`, and each
`SavedServer` row uses `deny_unknown_fields` (`src/ui/server_dialogs.rs`).
Writing a new field into a version-1 row would therefore produce a file
that the loader's own v1 branch — and any older build — quarantines, so
the offline subsystem raises the saved-source schema to version 2 instead.
A v2 envelope carries `schema_version: 2` and a `servers` array of rows
`{ type, name, url, source_id, source_incarnation }`, with
`source_incarnation` required and `deny_unknown_fields` retained at both
...
unknown-version envelope in place without rewriting or deleting it, so a
downgrade makes saved sources unavailable but never silently drops an
...
4. **Verify on the temp file.** The engine re-computes SHA-256 from the
   ...
   the per-track payload cap here, before any rename: a snapshot whose
   verified payload size strictly exceeds the cap fails `QuotaExceeded`
   and the temp is
   unlinked, while a size exactly equal to the cap passes. No rename has
...
6. **Commit.** Only after a successful rename does the cache row exist.
   ...
   The ordinary commit is the same atomic admission event the
   restart-adoption path performs, and it uses the same authority boundary:
   the boundary is not reserved for recovery. Immediately before the row
   insert the engine acquires the exact session commit authority atomically
   under the lifecycle state lock and retains it through the idempotent
   insert and the intent clear. That acquisition rechecks the registry
   incarnation, the exact `SourceId`, the exact adapter pointer, the session
   epoch, and the active lease, and evaluates the same synchronous,
   non-reentrant predicate that reads the exact adapter's current
   `offline_snapshot` capability and `OperationalLicence` under the same
   lock — the boundary `architecture/source-lifecycle.md` states as "at the
   database boundary, the registry verifies the same registry incarnation,
   source, adapter pointer, epoch, capability, and active lease under the
   lifecycle mutex, then acquires an opaque session-only permit retained
   through commit or rollback", and the same one `src/source_lifecycle.rs`
   implements as `acquire_session_commit_authority_if`. The predicate
   performs no filesystem, network, or registry work and never re-enters the
   lifecycle.

   Gating the ordinary commit on that authority is what keeps the decisive
   cancellation, replacement, and generation-ownership guarantees true on
   the hot path, not only under restart recovery. A source replacement,
   disconnect, retirement, shutdown, capability withdrawal, or licence
   revocation that wins between admission and the ordinary commit makes the
   acquisition return no authority, so the engine inserts no row and clears
   no intent: the job never becomes playable underneath authority that is
   no longer current, and the published file remains durably owned and
   non-playable under the pending intent — exactly as a blocked adoption
   leaves it — until a later pass adopts it once current authority is
   establishable or resolves it by recorded-file cleanup when the terminal
   gate is definitive. A cancellation that wins in the same window is a
   post-rename terminal state under the publish-intent protocol below: its
   verdict-first cleanup owns the file, and the ordinary commit, like
   adoption, is never reached. A replacement, disconnect, retirement, or
   shutdown that lands after the permit is acquired waits for the permit to
   drop and cannot invalidate the commit; the idempotent insert and intent
   clear complete under the authority the admission event selected, and any
   later retirement of the committed row runs through its own retirement
   protocol, never through this commit.

   What the preceding step guarantees at commit is
...
- Verify: `OfflineError::IntegrityMismatch` on digest mismatch;
  ...
  an expected digest; `OfflineError::QuotaExceeded` when the verified
  payload size
  strictly exceeds the per-track cap (equality passes). The temp file is
  unlinked; no cache row is created; nothing was ever renamed.
- Publish: `OfflineError::StorageUnavailable`. A failed rename leaves the
  ...
  rule of step 6.
- Commit: the row insert is admitted only under the exact session commit
  authority of step 6, so a source replacement, disconnect, retirement,
  shutdown, capability withdrawal, or licence revocation that wins between
  admission and the commit creates no cache row and clears no intent. The
  published file stays durably owned and non-playable under the pending
  intent, resolved by a later pass (adoption once current authority is
  establishable, recorded-file cleanup when the terminal gate is
  definitive); no destructive step runs. A cancellation in the same window
  is the post-rename terminal state governed by the publish-intent
  protocol below.
...
3. **Startup resolves a pending intent by inspection.** A job whose journal
   ...
   **Adoption third.** Adoption completes step 6 only for a
   ...
   authority. That revalidation is asynchronous and runs outside any
   lifecycle lock, exactly as a resumed job's does.

   **The playable insert is one atomic admission event.** A point-in-time
   revalidation taken before the publication barrier is not by itself an
   authorization to insert: the source can be replaced, disconnected,
   retired, or have its capability or licence revoked while recovery
   performs the filesystem barrier, and the stale job must not become
   playable underneath authority that is no longer current. Adoption
   therefore separates the asynchronous work from the admission it
   authorizes. It first completes the full platform publication barrier
   unlocked — the Unix ancestor-chain re-derivation and `fsync` pass and
   the snapshot-directory `fsync`, or the Windows rename-call barrier, per
   step 5 and the file-state bullets below — and then, immediately before
   the row insert, acquires an exact session commit authority atomically
   under the lifecycle state lock. That acquisition rechecks the registry
   incarnation, the exact `SourceId`, the exact adapter pointer, the
   session epoch, and the active lease, and evaluates a synchronous,
   non-reentrant predicate that reads the exact adapter's current
   `offline_snapshot` capability and `OperationalLicence` under the same
   lock; the predicate performs no filesystem, network, or registry work
   and never re-enters the lifecycle. It is the same admission boundary
   `architecture/source-lifecycle.md` states as "at the database boundary,
   the registry verifies the same registry incarnation, source, adapter
   pointer, epoch, capability, and active lease under the lifecycle mutex,
   then acquires an opaque session-only permit retained through commit or
   rollback", and the same one `src/source_lifecycle.rs` implements as
   `acquire_session_commit_authority_if`. On success the permit is retained
   across the idempotent row insert and the intent clear, so the commit is
   admitted under authority that is exact at the commit boundary and cannot
   be invalidated between validation and insert. No lifecycle lock is held
   over the asynchronous revalidation or the publication barrier, so
   filesystem and network work never run under the state lock.

   **Both race orders are closed, and each resolves without a destructive
   step.** (a) A replacement, disconnect, retirement, shutdown, or a
   capability or licence revocation that wins *before* the authority is
   acquired makes the acquisition return no authority: adoption creates no
   row, runs no unlink, retains the pending intent as the durable owner,
   and a later recovery pass retries — the non-destructive deferral
   below. (b) A replacement, disconnect, retirement, or shutdown that
   lands *after* the permit is acquired waits for the permit to drop and
   cannot invalidate the commit; the idempotent insert and intent clear
   complete under the authority the admission event selected, and any
   later retirement of the committed row runs through its own retirement
   protocol, never through this recovery's unlink. Staleness before
   admission rejects; replacement after admission waits.

   A job failing a definitive gate resolves as recorded-file
   ...
   current accepted generation and the commit authority are
   establishable — the same fail-closed retention the barrier arm below
   uses. That cleanup is reachable only when no committed row was
   ...
   - The file is present and its SHA-256 matches the journaled digest: the
     ...
     barrier holds does adoption acquire the exact session commit authority
     described above, and only with that authority retained does the engine
     complete step 6 — inserting the cache row; the
     `(source_key, track_key, snapshot_key)` key makes the insert
     idempotent — and clear the intent. If a required barrier cannot be
     completed, or the commit authority cannot be acquired, adoption
     creates no row and clears no intent: the pending intent remains the
     durable recovery/cleanup owner and a later recovery pass retries once
     both are establishable, so a committed Unix row never references a
     path whose rename entry is not durable.
   - The file is present and the digest does not match: the bytes are not
     ...
     intent, and restarts the job from zero, subject to the bounded
     publish-attempt budget (rule 6).
   - The file is absent: the rename never happened — or the published name
     ...
     from zero under the normal resumption rules — while the
     publish-attempt budget is not yet exhausted. At the budget it begins
     no further attempt: it records the terminal
     `Failed(PublishRetriesExhausted)` safe refusal of rule 6 and finishes
     cleanup.
4. **The gap is invisible downstream.** Between the rename and the commit
   ...
   per-track payload cap as any committed snapshot — the cap is enforced
   before
   the rename, so this window can never hold an over-cap file — and its
   lifetime is bounded by its durable owner rather than by a pass count.
   The file ends when a pass recognizes its committed row and clears the
   intent, when a verdict-owning cleanup completes the unlink and clears
   the intent, when adoption acquires its commit authority and completes
   the insert, or when the pre-rename file-absent resolution clears the
   intent. A pass that cannot establish current authority, acquire the
   commit authority, or complete the platform barrier makes no destructive
   change and leaves that same durable owner in place, so a durably owned,
   non-playable file may survive more than one recovery pass; every pass
   leaves it owned and non-playable, and no pass ever strands it without a
   durable owner.
5. **The crash-point matrix.** Every kill point in the publish window has
   ...
   | Power loss before or during the step-5 ancestor-durability pass, before the rename | No commit; final absent; the temp or some created ancestors may or may not have survived; a pending intent may be present. | File-absent resolution for any pending intent; on the next attempt the complete chain pass runs from the top before the rename, subject to the bounded publish-attempt rule (rule 6). | Nothing was published; re-syncing the chain is unconditional and cannot double-publish. |
   | Power loss after the rename, before the snapshot-directory `fsync` and the commit | Intent present; the published name may or may not have survived, because the rename's own entry was not yet synced; the ancestor chain is already durable from the pass. | Final present with the intent's digest → the adoption path first completes the interrupted publication barrier — on Unix re-deriving the recorded final path's complete ancestor chain and `fsync`-ing the snapshot directory that holds the already-renamed file, without repeating the rename — then acquires the exact session commit authority and, with it held, inserts the row and clears the intent; if the barrier or the authority acquisition fails, no row is created and the intent stays as the durable owner, retried on a later pass. Final absent → clear the intent and resume or restart per the journal while the publish-attempt budget is not exhausted; at the budget, record the `Failed(PublishRetriesExhausted)` safe refusal (rule 6). | Ancestors are durable before the rename, so only the rename's own entry is at stake; a barrier- or authority-blocked arm leaves the pending intent as the durable owner and may need more than one pass, and no committed Unix row exists until that entry is durable. |
   | Process crash after the rename, before the snapshot-directory `fsync` and the commit | Intent present; the published name resolves in the still-running kernel and matches the intent's digest; the rename's own entry is not yet durable; the ancestor chain is durable from the pass; job `Committing`. | Adoption first completes the pending publication barrier — on Unix re-derives the recorded final path's complete ancestor chain and `fsync`s the snapshot directory holding the already-renamed file, never repeating the rename — then acquires the exact session commit authority and, with it held, inserts the row and clears the intent; if the barrier or the authority acquisition fails, no row is created and the intent remains the durable owner, retried on a later pass. Because the barrier precedes the row insert, the recovery's own commit is durable and a subsequent power loss cannot lose the name. | The step-5 ordering is completed by recovery under a retained commit authority: a committed Unix row never exists before its publish-directory entry is durable. |
   ...
   | Intent `fsync`'d, before the rename | Intent present; temp present; final absent; job `Committing`. | File absent → clear the intent; resume or restart per the journal while the publish-attempt budget is not exhausted; at the budget, record the `Failed(PublishRetriesExhausted)` safe refusal (rule 6). | No publish without a commit; temp resume intact. |
   | Repeated intent-before-rename crashes reach the publish-attempt budget (including with the free global quota at zero) | Intent present, or just cleared, with the durable `publish_attempt` counter at `publish_attempt_budget`; no further attempt is admissible. | Terminal safe refusal: write `Failed(PublishRetriesExhausted)`, clear any pending intent, release reservations at terminal cleanup; no new intent is written. | The bounded lifecycle footprint is what lets the exhaustion record fit; the exhausted job never writes an unbudgeted lifecycle record. |
   | Rename applied, before the step-6 commit | Intent present; final present with the intent's digest; temp gone; job `Committing`; no verdict record. | Adoption path: digest match → complete the pending publication barrier first (on Unix re-derive the recorded final path's chain and `fsync` the snapshot directory of the already-renamed file; never repeat the rename), then acquire the exact session commit authority; with it held the idempotent row insert completes step 6 and the intent is cleared; a barrier or authority failure creates no row and retains the intent, retried on a later pass. | A row exists only after a verified rename and a completed durable publish barrier under a retained commit authority; an authority- or barrier-blocked file stays durably owned and non-playable, not an orphan. |
   | Rename applied, then a newly created ancestor entry lost for want of durability | Intent present; final absent (the published name did not survive); job `Committing`. | File-absent resolution: clear the intent; the job continues per the journal — resume or restart from zero, subject to the bounded publish-attempt rule (rule 6); missing ancestor directories are rebuilt under the ordinary creation rules. | On Unix, step 5's chain-durability ordering confines this row below the commit. On Windows the same crash before the commit resolves through this row; the same loss after the commit resolves through the post-commit loss row below. |
...
6. **The publish protocol is bounded, so its fixed reservation can never
   be outgrown by retries.** The crash-safe lifecycle representation is
   bounded, not append-unbounded: one job may begin at most a fixed,
   compile-time `publish_attempt_budget` publish attempts. Each attempt
   writes at most one publish-intent record and one resolution record-set —
   ...
   intent record it opens; recovery resolves only the open attempt's
   intent, never re-adopts a resolved attempt's records, and treats an
   intent whose attempt number is below the durable counter as already
   settled. Every file-absent resolution and every ordinary restart or
   retry that would begin a further publication first checks the counter:
   while `publish_attempt < publish_attempt_budget` the job may begin
   another attempt within its reservation, and at the budget it begins
   none. Instead it writes the fixed terminal
   `Failed(PublishRetriesExhausted)` record — the defined safe refusal,
   whose bytes its reservation already covers — clears any pending intent,
   and reaches terminal cleanup, where its reservations are released. A
   fresh download requested after that refusal is a new job with its own
   admission and its own lifecycle reservation; it never inherits the
   exhausted job's charge and never revives its attempt count. The budget
   is what makes the lifecycle footprint payload-independent and finite:
   `n` crash-and-retry cycles add lifecycle records only up to the cap, so
   a job at the global quota limit can still write every mandatory
   resolution and its exhaustion record.
...
without a durable owner. Every engine-owned file is either backed by a
row or held by a durable owner — a pending publish intent, a
terminal-verdict record with its delete intent, or a delete tombstone —
and a recovery pass either resolves that owner or leaves it standing
unchanged. A pass blocked on current authority, on the commit-authority
admission, or on a platform barrier is a non-destructive deferral, so a
durably owned, non-playable file may survive more than one pass until its
owner can resolve. The retry makes progress when a later pass can establish
the current authority, complete the barrier, and acquire the commit
authority (resolving by adoption), or when it can definitively establish a
terminal gate or a durable verdict (resolving by cleanup); between such
passes the file simply remains durably owned and non-playable. Ownership
ends only when a resolution above clears the intent or the staged delete
commits. The publish-window rows of the
...
1. **Quota is global, and the accounting covers every byte the engine
   ...
   side channel; it has two parts, both charged against the same global
    total as payload. The **segment records** are payload-derived: each
    committed segment appends one bounded, fixed-size record — the
    segment's offset and length, its SHA-256, and fixed framing that
    includes a checksum over the record's own body — and the
    engine never opens a new segment record for fewer than a fixed minimum
   number of payload bytes (it coalesces appended bytes into the open
   record), so the segment-record metadata a job can grow is a
    deterministic, bounded function of the payload it receives. The
    **lifecycle records** — the publish-intent, intent-clear, and
    terminal-verdict records — are a fixed, payload-independent overhead,
    each carrying the same per-record body checksum (a lifecycle record
    certifies no payload, so that checksum is its only integrity field),
    bounded by the capped publish-attempt budget of the
   ...
   lifecycle reservation** (`lifecycle_reserve`, sized to the whole bounded
   worst case), taken exactly once before the job receives
   its first byte and held for the job's whole life, rather than by the
   payload-derived segment bound. The reservation's unwritten portion
   survives every append, short read, verification, cancellation, deferred
   recovery, and repeated publication retry; its bytes convert to durable
   charges exactly once
   as the intent, intent-clear, and verdict records are actually written;
   and its unused capacity is released only at terminal cleanup, when no
   remaining lifecycle transition can need it. A retry never re-charges and
   never enlarges the reservation: its size bounds all admissible attempts,
   not one. A job sitting exactly at
   the global limit can therefore still write every mandatory `fsync`'d
   intent, clear, and verdict record its cleanup protocol requires — and
   the final record of an exhausted budget — without
   charging those bytes twice and without the count of retries consuming
   capacity its terminal cleanup still needs. In addition to the global
   quota, each snapshot is bounded by a **per-track byte cap** on its
   payload — its durable snapshot bytes, not its metadata — that is
   enforced
   **before the step-5 rename**, so a published-but-uncommitted file can
   ...
   - At admission, a job whose declared total (`Content-Length` when
     known) exceeds the payload cap fails `QuotaExceeded` before any
     network work.
     ...
     segment-record metadata those remaining payload bytes can grow**,
     against the engine's same-process quota accounting, the same global
     total an unknown-length response draws on, before it reads or writes
     a byte. Its single lifecycle reservation is charged at the same
     admission, exactly once for the job, and is sized to the whole bounded
     publish-attempt budget — so it is an upper bound on every lifecycle
     record the job can ever write across all attempts and retries; the
     payload-and-segment term never re-charges that allowance.
     ...
     records, and the combined reservation is an exact upper bound on the
      engine-owned bytes the job can make durable, segment records and the
      bounded lifecycle records included. The payload-and-segment
     reservation is charged to the global total at reservation time, and
     any unused remainder of it — the payload not received and the
     segment records not written — is released back to the global total,
     while the lifecycle reservation's unused portion stays charged until
     terminal cleanup (see the lifecycle rule above), so a
     declared-length job whose remaining payload plus its bounded
     segment-record metadata plus the budget-sized lifecycle reservation the
     free global quota cannot cover fails `QuotaExceeded` at admission
     ...
     oversubscribe it: with 10 MiB free, a declared job is admitted only
     when its declared payload plus the bounded segment metadata those
     bytes can grow plus the budget-sized lifecycle reservation all fit the
     free global quota, so a declared job whose payload alone is 10 MiB is
     refused at admission — its metadata and lifecycle records would push
     the engine-owned footprint past the free total — while a smaller
     declared job that leaves room for them is admitted, and a concurrent
     declared job then sees only the remaining free global quota and is
     refused if it cannot cover its own combined reservation. A resumed job
     reserves only its not-yet-durable payload
     remainder and the segment metadata for the records it has still to
     write, plus the remainder of its bounded lifecycle obligation
     reconstructed from the durable journal (see the restart rule below),
     which never exceeds the budget-sized reservation the job already held,
     because the payload and the records already durable on its temp file
     are already charged.
   - A response of unknown length is bounded before each append, not
     ...
     above), the job atomically **reserves** the largest payload `P` such
     that `P` fits the track's remaining payload cap **and**
     `P + metadata(P)` fits the free global quota — that is,
     `P ≤ remaining per-track cap` and
     `P + metadata(P) ≤ free global quota`, where
     `metadata(P)` is the bounded segment-record metadata `P` can grow —
     against the engine's same-process
     quota accounting before it reads or writes any byte, and bounds the
     segment to the reserved payload. The job's lifecycle reservation was
     charged once before its first append and is already held against the
     global total, so the free global quota this formula tests is net of
     it: an append reservation covers incremental payload and segment
     metadata only and never re-charges the lifecycle allowance. The
     reservation is sized to the whole bounded attempt budget, so a retry
     neither re-charges nor enlarges it and this formula is unchanged
     across attempts. The
     payload-and-segment reservation is charged to the
     ...
     the first reservation takes the largest payload that leaves room for
     its own segment metadata, and the
     second then sees only the free quota that remains to it. After the
     ...
     unused remainder of that payload-and-segment reservation (a short
     read, a failed or cancelled append, and any
     ...
     were not made durable and is never released twice. The lifecycle
     reservation is not released here: it stays charged until its bytes
     are made durable or the job reaches terminal cleanup. Cross-process
     ...
     job. The two bounds are separate invariants: a job's durable payload
     never exceeds the per-track payload cap, and its durable payload plus
     its charged journal/sidecar metadata — segment records and lifecycle
     records — plus its outstanding reservations never exceeds the global
     total.
   - When the reservation is zero — or the free global quota can no longer
     ...
     the job does not fail blindly. (The lifecycle reservation does not
     enter this test: it was charged once and is still held, so the
     mandatory intent, intent-clear, and verdict records remain writable
     from it even when the free global quota is zero.) A bounded read that
     fills the
     ...
     payload actually exists. A response whose payload size equals, not
     exceeds,
     the cap is therefore treated as complete and proceeds to the
     verified-size check, which passes at exact equality; the quota
     failure is reserved for a stream that really does carry more bytes
     than the cap allows.
   - Independently of any streaming estimate, the verified size on the
     temp file is checked against the payload cap before the rename
     (step 4/5). A
     snapshot whose verified payload size strictly exceeds the cap resolves
     like
     any quota failure — the temp is cleaned per its state, no rename
     occurs, and no
     ...
     window bounded by the per-track payload cap, as rule 4 of the
     publish-intent
     protocol assumes.
   - Journal/sidecar metadata is charged and released by the same rules as
     payload, so it is never an uncharged durable write and is never
      double-released. On restart the engine trusts no process-local
       ...
       metadata it owns from the journal's framed, checksum-valid records, charging
       that total before any job resumes, so temp and journal bytes already on
       disk are counted exactly once. The re-derivation first validates each
       sidecar's valid record prefix — the first position where the fixed
       framing does not yield a complete record, or where a complete-length
       frame's per-record checksum fails over its body, ends the prefix — and
       resolves a mid-append crash's casualty before admitting
       anything: it durably truncates the tail to the valid prefix, or, when
       truncation cannot be made durable, charges the tail's full physical
       length to the owning job until a later pass reclaims it, and an
       inability to read, validate, or truncate a sidecar fails that job's
       admission closed with the bytes held charged at their observed length —
       or at the job's pre-crash upper bound when even their length cannot be
       observed — until they become reclaimable. Framing length alone never
       admits a record: a power loss between the framing bytes reaching disk
       and every body sector doing so can leave a full-length frame with a
       torn or corrupted body, the segment record's SHA-256 certifies the
       payload bytes and not the record that carries it, and the lifecycle
       records certify no payload at all — so their intent, verdict, and
       attempt-counter fields would otherwise be trusted from unverified
       bytes. Every record therefore carries one checksum over its own body,
       admission of a record requires that checksum to verify, and the first
       failure — incomplete framing or failed checksum — ends the prefix, so
       recovery never trusts a full-length torn frame's lifecycle fields any
       more than a short tail's. The rule is one rule for
       segment and lifecycle records alike, and it is engine-local: prefix
       validation and truncation need no source authority, discard no
       checksum-valid record, and touch no durable owner, so an
       authority-deferred recovery
       pass performs it too and never defers with unaccounted bytes on disk.
      `journal_bytes` alone is not the whole
     obligation: it counts only the records already durable. The engine
     also **reconstructs the job's remaining lifecycle obligation** from
     the durable journal state and the durable `publish_attempt` counter —
     the records for the attempts still admissible under
     `publish_attempt_budget`, plus the budget-exhaustion record, less the
     lifecycle records already durable — and re-charges that remainder
     before it admits any competing job; a resumed
     job likewise re-derives its remaining payload-and-segment allowance
     from the journal. Because the attempt count is capped, that remainder
     is at all times bounded by, and charged together with the durable
     lifecycle bytes to exactly, the budget-sized `lifecycle_reserve` the
     job already held, so the reconstruction can never demand bytes the
     reservation did not cover and a competing job cannot
     be admitted against a budget that already spent the cleanup records'
     bytes. Eviction and retirement release a
     ...
     emergent: because a segment record is fixed-size and a new segment
     record opens only every minimum-segment worth of payload, a segment
     record is covered by the payload-derived part of the reservation,
     while the single lifecycle reservation is sized to the capped
     publish-attempt budget and held from admission, so
     the mandatory intent, intent-clear, and verdict records always fit
     and repeated retries add no lifecycle bytes beyond that bound;
     no admitted job
     can grow metadata its reservation did not cover, and a job that
     cannot cover the next segment record fails `QuotaExceeded` before that
     record — or any byte it would certify — is made durable.
   - **Worked invariant check.** Two unknown-length jobs `A` and `B`
     share an exact total quota. `A` takes its single `lifecycle_reserve`
     before its first append; that append then reserves
     `P₁ + metadata(P₁)`, and `B` observes only the free total left after
     `A`'s lifecycle reservation and segment reservation are charged, so
     the two never jointly overcommit. `A` makes segment 1 durable — the
     payload and its one segment record transfer to durable charges — and
     its second append reserves `P₂ + metadata(P₂)` against a free total
     that still excludes `A`'s remaining lifecycle obligation. A short,
     failed, or cancelled append releases only `A`'s unused
     payload-and-segment allowance; `A`'s lifecycle reservation stays
     charged, so its mandatory records still fit when the free total is
     zero. At the post-rename terminal transition `A` writes its verdict
     and intent-clear from that reservation — each byte converting from
     reservation to durable `journal_bytes` exactly once — and releases
     only what is left at terminal cleanup. A crash mid-job leaves no
     process-local reservation: on restart the engine re-charges `A`'s
     durable journal bytes and reconstructs the remaining lifecycle
     obligation from the journal before admitting `B`, so `A`'s cleanup
     records still fit under the recovered accounting. Across all of it,
     durable bytes plus outstanding reservations never exceed the quota,
     the lifecycle overhead is charged once per job, and the mandatory
     cleanup records always fit.

     **Bounded-retry check at exact quota.** Place `A` alone at the exact
     total: admission charges `A`'s `lifecycle_reserve` and its
     payload-and-segment reservation, leaving the free global total zero,
     so no competing job can be admitted against bytes `A` has not yet
     written. `A` now runs the rejection's counterexample: append and
     `fsync` intent `I₁`, crash before the rename, wake to a file-absent
     resolution that clears with `C₁`, resume, append `I₂`, crash again,
     clear `C₂`, and so on. Each cycle writes its intent and clear from
     `A`'s single reservation; the durable `publish_attempt` counter
     advances with each intent and stops the job at
     `publish_attempt_budget`. After at most that many cycles `A` writes
     its terminal `PublishRetriesExhausted` record — from the same
     reservation — and reaches terminal cleanup, where all its
     reservations are released. In no cycle does `A` need a byte beyond
     its reservation, so the free global total being zero never blocks a
     mandatory record. If instead `A` succeeds on attempt
     `k ≤ publish_attempt_budget`, it writes that attempt's intent and
     resolving clear and completes; a cancellation landing after the
     rename writes the verdict and then the clear. Every branch is covered
     by the same reservation: durable lifecycle bytes never exceed
     `lifecycle_reserve`, no lifecycle write is uncharged, and `n` retries
     add no durable lifecycle bytes beyond the budget's bound. The
     two-job walk above is unchanged by retries, because each job's
     lifecycle reservation is fixed at admission.

      **Torn-tail check at exact quota.** Place `A` alone at the exact
      total — free global zero once its reservations are charged — and
      crash it mid-append to a sidecar record: part of the record's bytes
      reached the disk, its `fsync` never completed, and the process died.
      The crash takes either shape the medium allows: a short tail whose
      framing never completed, or a full-length frame — the length and
      framing bytes reached disk — whose interior body sectors never did.
      The restart pass finds a torn payload tail past the journaled
      offset, a torn or checksum-failed record tail past the last valid
      sidecar record,
      and no surviving reservation. Before anything is admitted it
      validates `A`'s sidecar prefix by framing and per-record checksum,
      durably truncates the record tail to
      that prefix — and the temp to the journaled offset, per the
      resumption rule — and re-derives the accounting from valid
      records alone: `A` is charged its valid-prefix `journal_bytes` plus
      its reconstructed lifecycle obligation, and the sidecar's physical
      length equals the charged prefix again. The full-length frame ends
      the prefix exactly as a short tail would — its checksum fails, so it
      certifies no byte and its intent, verdict, or attempt-counter fields
      are never trusted from torn bytes — and truncate-or-charge resolves
      both shapes by the same rule. `B` — admitted only after
      that pass — sees a free global total computed from bytes that are
      actually free: the tail `A`'s torn write physically occupied was
      reclaimed before `B` could be admitted against it, so no competitor
      ever oversubscribes space a torn append still holds. If `A`'s
      publication recovery is instead deferred because its source
      authority is unavailable — the pending intent retained, no
      destructive step run — the same pass has already truncated the tail,
      because the truncate-or-charge rule is engine-local: it needs no
      authority, discards no checksum-valid record, and leaves no durable owner
      touched, so a deferral never persists with unaccounted bytes, and
      every later pass re-validates the prefix so a fresh tail — short or
      full-length — is
      truncated or charged before anything is admitted. Had the
      truncation failed instead, `A` fails admission closed and the
      tail's observed length stays charged until reclaimable: the quota
      is never told bytes are free while they are still on disk.
...
| Quota exceeded before publish (a declared total that exceeds the payload cap, a declared-length admission or unknown-length append whose payload plus its bounded segment metadata the free global quota cannot cover, or a verified payload size that exceeds the cap) | Job fails terminally with `QuotaExceeded`; temp cleaned, no rename, no row, and no published-but-uncommitted file. Any payload-and-segment reservation taken at admission or before an append but not made durable — payload and the journal/sidecar records that were never written — is released back to the global total exactly once before the job terminates; the job's single lifecycle reservation is released at terminal cleanup, when no remaining lifecycle transition needs it; metadata already made durable remains charged until the job's journal is removed at terminal cleanup. A response whose payload size exactly equals the cap is not a quota failure: the bounded read's end-of-stream signal, or a non-durable one-byte probe, shows no further payload, and the verified-size check passes at equality. |
| Publish-attempt budget exhausted by repeated crashes in the rename window (including repeated intent-before-rename crashes with the free global quota at zero) | Terminal safe refusal `Failed(PublishRetriesExhausted)`: no further publish attempt begins, any pending intent is cleared, the exhaustion record is written from the already-charged bounded lifecycle reservation — never from a fresh or uncharged charge — and reservations are released at terminal cleanup. A subsequently requested download is a new job with its own admission and lifecycle reservation. |
| Process crash mid-sidecar-append leaves a torn partial record — segment or lifecycle — past the last valid record, or a full-length record whose body fails its per-record checksum | Startup accounting validates the sidecar's record prefix by framing and per-record checksum before any admission and resolves the failed tail by truncate-or-charge: durably truncated to the valid prefix when possible — a torn tail, short or full-length, certifies no byte, so truncation discards nothing certified and a corrupted frame's lifecycle fields are never trusted — otherwise charged at its full physical length to the owning job until a later pass reclaims it. A sidecar that cannot be read, validated, or truncated fails the affected job's admission closed with its bytes held charged at the observed length, or the job's pre-crash upper bound when even the length cannot be observed. The rule is engine-local, so an authority-deferred pass still performs it and never defers with unaccounted bytes on disk. |
...
| Crash between the publish rename and the row commit | Startup recovery resolves the journaled publish intent by the rule-3 order: recognize a committed row first; otherwise adopt (complete the commit) only for a publication-eligible job whose exact session commit authority is acquired atomically after the publication barrier; and resolve a job that definitively establishes a terminal gate — a durable verdict, or a currently established accepted generation that denies or withdraws the capability, replaces the incarnation, or revokes the licence — as recorded-file cleanup: the idempotent unlink of the published file, then the intent clear. A pass blocked on current authority, the commit-authority admission, or a platform barrier creates no row, runs no unlink, and retains the pending intent as the durable owner for a later pass. Never a playable row without verified bytes and never a file without a durable owner; a durably owned, non-playable file may therefore survive until a later pass resolves it. |
| Source replacement, disconnect, retirement, shutdown, capability withdrawal, or licence revocation wins between admission and the ordinary step-6 commit | The ordinary commit acquires the exact session commit authority immediately before the idempotent row insert and retains it through the insert and the intent clear, exactly as the adoption path does. If the acquisition returns no authority, the commit creates no row and clears no intent: the job is never promoted playable underneath stale authority, and the published file stays durably owned and non-playable under the pending intent until a later pass adopts it once current authority is establishable or resolves it by recorded-file cleanup when the terminal gate is definitive. A replacement or shutdown after admission waits for the retained permit and cannot invalidate the commit. |
...
3. Persists no row that points at a missing or partial file. Promotion to a
   ...
   generation), completes the platform publication barrier, acquires an
   exact session commit authority atomically at the final admission, and is
...
one, and the saved-source `servers.json` envelope moves to version 2 as
described under [restart
authorization](#authenticated-resumable-download-jobs); the two version
numbers are distinct and each is idempotent on retry.
...
incarnations through the v1→v2 `servers.json` envelope, and raises the
application schema version and the saved-source envelope version whether
or not any source ever opts in — so byte-level identity
