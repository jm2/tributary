...
def counter_paragraph(*, overall=(1, 4, 25.0), baseline=(1, 2), corrective=(0, 1),
                      engineering=(0, 1), archived=(223, 226, 98.7),
                      include_labels=True):
...
class BacklogConsistencyTests(unittest.TestCase):
    ...
    def test_main_rejects_malformed_ledger_snapshot(self):
        ...
        self.assertEqual(
            checker.main(["--root", str(root), "--ledger", str(malformed), "--quiet"]), 1)
