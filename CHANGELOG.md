...
- **Windows PE-import inspection no longer uses positional argument binding, but the reported
  ...
  control value as another target and rejecting it with
  `PE import-inspection target must be an absolute existing DLL or EXE`. Both production calls now
  bind every argument by its declared name. The regression contract checks the declaration and
  each invocation independently, so one correctly named call cannot mask a positional sibling.
  PowerShell 7 parses the complete script, all 11 focused Windows contracts pass, and locked debug
