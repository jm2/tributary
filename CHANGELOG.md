...
- **Root authority is not yet pinned through every reconciliation mutation** — Marker and mount observations are checked around traversal and authority transitions, but individual watcher/reconciliation writes and deletions are not held to one opened root handle or an equivalent mount generation on every supported platform. A same-marker remount or copied-marker ABA after traversal can therefore survive path-based probes; Linux has additional mount-instance checks but still has a post-traversal mutation window, and portable Unix/Windows pinning remains incomplete. This remains open under P0.2.
- **Auth secrets travel in some stream/cover-art URLs** — Tributary's debug logger redacts `api_key=` and `X-Plex-Token=` for Jellyfin and Plex, but the URLs handed to GStreamer's `playbin3` (and to the album-art HTTP fetcher) are the unredacted originals. Those tokens may therefore appear in GStreamer's own debug output and in upstream HTTP access logs. Subsonic normally uses a salted-MD5 token and salt; its legacy compatibility fallback instead places a reversibly hex-encoded password in the URL and is permitted only over HTTPS. That fallback remains a URL-secret exposure. Mitigation: keep `GST_DEBUG` unset, prefer HTTPS endpoints, and complete the P1.6 proxy/ticket work before treating receiver playback as credential-safe.
