...
- **Credentials are stripped from errors and logs** — Request URLs are removed before an HTTP error is retained or displayed, URL user-info and backend query credentials are rejected or redacted, DAAP session IDs are no longer logged, and MPD command arguments are no longer written to the log.
