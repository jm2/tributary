...
- **MPD playback could race commands and report guessed state** — MPD output now owns one ordered worker and persistent connection per load, tracks the exact `addid`/`playid` song identity, and publishes state, position, duration, errors, and completion from authoritative receiver status. Pause resumes the same song, an explicit remote Stop remains restartable, natural queue exhaustion emits end-of-track, and replacement by another MPD client relinquishes ownership without clearing that client's queue. Protocol input, addresses, media URIs, idle I/O, and whole operations are bounded; desynchronized connections are discarded instead of reused.
