...
### Security
- **Authenticated requests are pinned to an exact origin** — Every HTTP client that carries a credential (Subsonic, Jellyfin, Plex, DAAP, authentication, and artwork) now refuses to follow a redirect to a different scheme, host, or port, and never sends a `Referer`. Previously a redirect could walk a credential to an attacker-controlled host or downgrade it to plaintext HTTP.
- **Credentials are stripped from errors and logs** — Request URLs are removed before an HTTP error is retained or displayed, URL user-info and backend query credentials are rejected or redacted, DAAP session IDs are no longer logged, and MPD command arguments are no longer written to the log. Failures involving MPD server replies or authenticated media URLs are also reduced to opaque categories before they can reach player events or diagnostics.
...
- **Auth secrets still exist in generic stream and cover-art URLs** — `Track.stream_url`, its UI copy, local `playbin3`, AirPlay `uridecodebin`, and the album-art fetch path still hold the unredacted backend URL. The app-owned artwork fetch is exact-origin and URL-free on error, but credentials can still appear in upstream access logs; local and AirPlay decoding may also expose them through GStreamer diagnostics. For the supported current backend credential shapes, Chromecast and MPD now receive only opaque proxy tickets and never the upstream credential. Subsonic normally uses a salted-MD5 token and salt; its legacy compatibility fallback instead places a reversibly hex-encoded password in the URL and is permitted only over HTTPS, so that fallback remains especially sensitive. Keep `GST_DEBUG` unset and prefer HTTPS until P1.4 moves local/AirPlay fetching behind the app-owned policy and P1.6/P3.1 resolve credentials only at playback time.
- **Media tickets have revocation but no TTL** — Each opaque URL is a replayable bearer for one fixed media item and arbitrary byte ranges until Tributary revokes it; it is not a one-shot token. Revocation blocks new route lookups but does not cancel a response the proxy already admitted, which may finish streaming. A protected MPD load from an unspecified address, or a scoped/link-local IPv6 route, fails closed rather than exposing the upstream URL. TTL and credential-free generic track values remain tracked in P1.6/P3.1.
